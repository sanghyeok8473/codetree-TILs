...
# B가 A의 일부만큼 겹치거나 아예 겹치지 않으면 A의 넓이가 정답, 포합되어있으면 0, 한 줄로 깔끔하게 지우면 남은 부분의 넓이가 정답(일부만 잘라서는 안됨)

if a[0] >= b[0] and a[2] <= b[2] and a[1] >= b[1] and a[3] <= b[3]: # a가 b에 포함된 경우
    print(0)    
elif a[0] >= b[0] and a[2] <= b[2] and (b[1] <= a[1] <= b[3] or b[1] <= a[3] <= b[3]) or a[1] >= b[1] and a[3] <= b[3] and (b[0] <= a[0] <= b[2] or b[0] <= a[2] <= b[2]): # 가로 한 줄이 깔끔하게 밀리는 경우 또는 세로 한 줄이 깔끔하게 밀리는 경우, 남은 a의 넓이 출력
        cnt = 0
